...
namespace BNG
{
    ...
    {
        [Tooltip(
            "Path of the directory where handposes should be stored. Tip : Keep these in a 'Resources' directory so you can use Resources.Load().")]
        ...
        [SerializeField] private HandPose currentPose;
        public HandPose CurrentPose
        {
            get => currentPose;
            set
            {
                bool poseChanged = currentPose != value;
                currentPose = value;
                if (poseChanged)
                {
                    OnPoseChanged();
        ...
        [Tooltip(
            "If true the local rotation of each bone will be updated while changing hand poses. This should generally be true if you are adjusting a hand pose.")]
        public bool UpdateJointRotations = true;

        [Tooltip(
            "If true the local position of each bone will be updated while changing hand poses. Typically this will be false as joints only adjust their rotations.")]
        public bool UpdateJointPositions = false;

        [Tooltip(
            "If true the local position of the wrist will be updated. Useful if you need to move the entire hand.")]
        public bool UpdateWristPosition = true;

        // Hand Pose Transform Definitions
        public HandPoseDefinition HandPoseJoints
        {
            get { return GetHandPoseDefinition(); }
        }
        ...
        {
            return ThumbJoints.Count + IndexJoints.Count + MiddleJoints.Count + RingJoints.Count + PinkyJoints.Count +
                   OtherJoints.Count + (WristJoint != null ? 1 : 0);
        }
        ...
        {
            return GetTip(ThumbJoints);
        }

        public Transform GetIndexTip()
        {
            return GetTip(IndexJoints);
        }

        public Transform GetMiddleTip()
        {
            return GetTip(MiddleJoints);
        }

        public Transform GetRingTip()
        {
            return GetTip(RingJoints);
        }

        public Transform GetPinkyTip()
        {
            return GetTip(PinkyJoints);
        }
        ...
        public virtual void UpdateJoint(FingerJoint fromJoint, Transform toTransform, bool doLerp, bool updatePosition,
            bool updateRotation)
        {
            UpdateJoint(fromJoint, toTransform, doLerp ? Time.deltaTime * AnimationSpeed : 1, updatePosition,
                updateRotation);
        }

        public virtual void UpdateJoint(FingerJoint fromJoint, Transform toTransform, float lerpAmount,
            bool updatePosition, bool updateRotation)
        {
            // Invalid joint
            ...
            if (lerpAmount != 1)
            {
                if (updatePosition)
                {
                    toTransform.localPosition =
                        Vector3.Lerp(toTransform.localPosition, fromJoint.LocalPosition, lerpAmount);
                }
                if (UpdateJointRotations)
                {
                    toTransform.localRotation =
                        Quaternion.Lerp(toTransform.localRotation, fromJoint.LocalRotation, lerpAmount);
                }
            }
        ...
        public virtual void SavePoseAsScriptablObject(string poseName)
        {
            ...
            AssetDatabase.CreateAsset(poseObject, fullPath);

            AssetDatabase.SaveAssets();

            AssetDatabase.Refresh(ImportAssetOptions.ForceUpdate);

            EditorUtility.SetDirty(poseObject);
    ...
    Debug.Log("Scriptable Objects can only be saved from within the Unity Editor. Consider storing in another format like JSON instead.");
        ...
        public virtual void CreateUniquePose(string poseName)
        {
            ...
            while (exists)
            {
                ...
                exists = File.Exists(ResourcePath + formattedPoseName + ".asset");
        ...
        {
#if UNITY_EDITOR
            var poseObject = Editor.CreateInstance<HandPose>();
